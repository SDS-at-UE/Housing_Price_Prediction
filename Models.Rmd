...
for (i in 1:length(housetmp)) {
  ...
  for (j in 1:length(col)) {
    ...
    if (abs(z[j]) > 7) {
      ...
      cat("Row",j,"was taken out.\n")
      }
    }
  cat("For", names(housetmp[i]), ", the outliers taken out were index", outliers, "\n")
}
house <- housetmp1
# 
# for (i in names(housetmp)[2:length(names(housetmp))]) {
#   housetmp1[i] <- housetmp[i]
# }
# housetmp <- housetmp1
```
...
x_train <- model.matrix(SalePrice~.-1, train)[,-80]
y_train <- train[80]$SalePrice
x_test <- model.matrix(SalePrice~.-1, test)[,-80]
y_test <- test[80]$SalePrice
```
...
```{r}
mean((y_test - cvpred)^2)
mean((y_test - ridpred)^2)
